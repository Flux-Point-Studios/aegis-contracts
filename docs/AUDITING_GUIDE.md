...
- All shared libs under [`contracts/lib/aegis/`](../contracts/lib/aegis/) — `types.ak`, `pricing.ak`, `pool.ak`, `oracle.ak`, `validation.ak`, plus test helpers.
...
1. **Read the audit report.** [`audit/SECURITY_AUDIT_REPORT.md`](audit/SECURITY_AUDIT_REPORT.md) — 24 findings discovered by 3 rounds of internal red-team. 22 closed, 2 open (both Info-severity, scope-related). Pay attention to:
   - The "Hash rotation history" at the top — explains the 5 deployments (v0..v5).
   - Each finding's writeup — including Round-2/Round-3 sections that reference live preprod tx hashes proving the exploit worked pre-fix.
   - The lessons captured at the end of each Round-2/3 finding.

2. **Read the architecture doc.** [`ARCHITECTURE.md`](ARCHITECTURE.md) — the on-chain mental model, datum/redeemer schemas, and cross-cutting design choices.

3. **Build and run the test suite.**
   ```bash
   cd contracts
   aiken check        # 186 tests, all green
...
5. **Verify Round-2/3 exploit txs.** Each Round-2 / Round-3 finding cites a specific preprod tx hash where the pre-fix attack succeeded. Visit them on cardanoscan to confirm:
...
1. **`pool.ak`** — the largest validator and the single point of fee enforcement. Most invariants live here. Pay particular attention to:
   - The `policy_output_matches_underwrite` and `batch_policies_match_totals` helpers (A-021, A-022, A-025 fixes).
...
2. **`policy.ak`** — per-policy lifecycle. A-008 (canonical pool routing), A-009 (enterprise-only payout), A-010 (in-the-money cancel guard), A-012 (uniform oracle in batch).
3. **`lib/aegis/pool.ak`** — `verify_underwrite_datum`, `verify_claim_datum`, `verify_add_liquidity_datum`, `verify_remove_liquidity_datum`. The datum-transition algebra. Pay attention to non-negativity guards (A-024 lessons).
4. **`lib/aegis/pricing.ak`** — `is_premium_adequate`, `calculate_protocol_fee`, `calculate_treasury_cut`. Math-level checks. A-014 (multiplication-form ratio) lives here.
5. **`lib/aegis/oracle.ak`** — `find_oracle_datum`, `find_oracle_output`. A-016 (canonical Charli3 script-hash binding) lives here.
6. **`lib/aegis/validation.ak`** — shared helpers (signed_by, validity-range bounds, `find_canonical_pool_output`, `sum_lovelace_to_enterprise_pkh`). A-008/A-009/A-013 helpers live here.
7. **`lp_token.ak`** — small. Mint policy gates LP token minting on pool consumption. Should be quick to read.
8. **`pool_nft.ak`** — one-shot mint policy. A-011 fix. Should be quick to read.
...
To attempt an attack against the live v5 deploy on preprod:
...
Use the existing finding numbering — next free is **A-026**.
...
- **Aiken LOC:** ~2,500 across validators + libs.
- **Compiled validator sizes:** policy ~3.1 KB, pool ~5.9 KB, lp_token ~0.5 KB, pool_nft ~0.5 KB.
- **Test count:** 186 (green-path).
- **Findings to date:** 24 (3 rounds of internal red-team).
